  ...
  <div class="contract-detail-wrap">
    <div class=" content-top-black content-padding">
      ...
      <div class="node-header">
        ...
        <div class="node-header-right">
          <span class="green vote-status" v-if="detailInfo.status == 2 || detailInfo.status == 6">{{ $t('nodeStatus.' +
            [detailInfo.status]) }}</span>
          <span class="yellow vote-status yellow-status" v-else-if="detailInfo.status == 3 || detailInfo.status == 4">{{
            $t('nodeStatus.' + [detailInfo.status]) }}</span>
          <span class="yellow vote-status yellow-status" v-else-if="detailInfo.status == 7">{{ $t('nodeStatus.' +
            [detailInfo.status]) }}</span>
          <span class="red vote-status red-status" v-else-if="detailInfo.status == 1">{{ $t('nodeStatus.' +
            [detailInfo.status]) }}</span>
          <span class="lightgray vote-status gray-status" v-else-if="detailInfo.status == 5">{{ $t('nodeStatus.' +
            [detailInfo.status]) }}</span>
        </div>
      </div>
      <!-- TODO 验证节点 详情 从新设计 -->
      <div class="node-static-box">
            ...
            <List class="node-left" :inline="true">
              ...
              <Item v-if="detailInfo.isInit" :vertical="true" :label="$t('nodeInfo.selfstake') + ' (TURN)'"
                ...
                <p class="Gilroy-Medium">
                  ...
                  <span v-if="(detailInfo.status == 4 || detailInfo.status == 5) &&
                      detailInfo.totalValue > 0
                      " class="fontSize13 onPending">({{ $t('nodeInfo.freezing') }})</span>
                </p>
              </Item>
            </List>
          </div>
        ...
        <div class="node-static-right-box" v-if="!detailInfo.isInit">
          ...
          <div class="yield-box">
            <p v-if="detailInfo.status == 4 || detailInfo.status == 5" class="value">
              --%
            </p>
    ...
    <div class="content-padding">
      <div class="tabs">
        ...
        <el-button size="medium" v-if="!detailInfo.isInit" :class="{ active: tabIndex == 4 }" @click="tabChange(4)">{{
        ...
        <div v-show="tabIndex == 2">
          ...
          <div class="table">
              ...
              <el-table :data="tableData" style="width: 100%" key="firstTable" size="mini" v-loading="loading.block">
                <el-table-column :label="$t('menu.block')">
                  <template slot-scope="scope">
                    <router-link class="blue cursor" :to="getBlockUrl(scope.row.number)">{{ scope.row.number
                    }}</router-link>
                  </template>
                </el-table-column>
            ...
            <div class="pagination-box">
              <el-pagination background @size-change="handleBlockSizeChange" @current-change="handleBlockCurrentChange"
