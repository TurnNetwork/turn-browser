<template>
  <div class="trade-list-wrap proposal-list">
    <div class="page-title fontSize34">{{ $t('menu.proposal') }}</div>
    <div class="sub-title">
...
<!--      </div>-->
    ...
    <div class="table" :class="{ 'no-data': !tableData.length }">
      <el-table :data="tableData" v-loading="loading" style="width: 100%" key="firstTable" size="mini"
        :empty-text="$t('tradeAbout.noProposalData')">
        ...
        <el-table-column :label="$t('tradeAbout.proposalTitle')" min-width="180">
          <template slot-scope="scope">
            <div class="flex-special" @click="goDetail(scope.row.proposalHash)">
              ...
              <p class="cursor blue ellipsis fontSize15" v-else-if="!scope.row.topic && scope.row.type == 1">
                {{ $t('proposalOption.' + [scope.row.type]) }}-{{ scope.row.pipNum }}</p>
              <p class="cursor blue ellipsis fontSize15" v-else-if="!scope.row.topic && scope.row.type == 2">
                {{ $t('tradeAbout.versionUp') }}-V {{ scope.row.newVersion }}</p>
              <p class="cursor blue ellipsis fontSize15" v-else-if="!scope.row.topic && scope.row.type == 4">
                {{ $t('proposalOption.' + [scope.row.type]) }}-{{ scope.row.pipNum }}</p>
              <p class="cursor blue ellipsis fontSize15" v-else-if="!scope.row.topic && scope.row.type == 3">
                {{ $t('proposalOption.' + [scope.row.type]) }}-{{ scope.row.pipNum }}</p>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column :label="$t('tradeAbout.status')" width="120">
          <template slot-scope="scope">
            <span class=" "
              :class="{ yellow: scope.row.status == 1, red: scope.row.status == 3 || scope.row.status == 6, green: scope.row.status == 2 || scope.row.status == 4 || scope.row.status == 5 }">{{
                $t('proposalStatus.' + [scope.row.status]) }}</span>
          </template>
        </el-table-column>
          ...
          <template slot-scope="scope">
            <div class="flex-special content progress percent60">
              ...
              <div class="percentage"
                :style="{ 'width': ((scope.row.curBlock - scope.row.inBlock) / (scope.row.endVotingBlock - scope.row.inBlock)) * 100 + '%' }">
              </div>
      ...
      <div class="pagination-box" v-if="paginationFlag">
        <el-pagination background @size-change="handleSizeChange" @current-change="handleCurrentChange"
          ...
          layout="sizes, total, prev, pager, next" :total="pageTotal" :pager-count="9"></el-pagination>
      </div>
    </div>
  </div>
</template>
