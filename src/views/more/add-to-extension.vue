<template>
  <div class="content-wrap" :style="{ 'min-height': clientHeight + 'px' }">
    <vue-particles color="#2E2E2E" :particleOpacity="0.7" :particlesNumber="80" shapeType="circle" :particleSize="4"
      ...
      :hoverEffect="true" hoverMode="grab" :clickEffect="false" clickMode="repulse" class="lizi"></vue-particles>
    ...
    <section class="content" v-if="config">
      ...
      <div class="detail">
        ...
        <template v-if="!status.isMobile">
          <button v-if="status.accounts.length" @click="addToNetwork" class="btn add" :disabled="loading.add"
            :class="{ active: loading.add }">
            {{ $t('extension.form.add') }}
          </button>
          <button v-else @click="connect" class="btn connect" :disabled="loading.connect"
