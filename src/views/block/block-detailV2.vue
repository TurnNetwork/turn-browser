<template>
  <div class="block-detail-wrap">
    ...
    <div class="information">
      <!-- 基础交易 -->
      <List :title="$t('blockAbout.blockInformation')" :border="false">
        ...
        <Item :label="$t('blockAbout.producer')" v-if="detailInfo.txType != '4000'">
          <p class="mobile-block-text" v-if="windowWidth < 750">
            ...
            <span class="mobile-block-text" style="line-height: 16px">【{{ timeDiffFn(detailInfo.serverTime,
              detailInfo.timestamp)
            }}{{ $t('tradeAbout.before') }}】</span>
          </p>
          <p v-else>
            <router-link class="blue cursor" :to="getNodeUrl(detailInfo.nodeId)">{{ detailInfo.nodeName }}</router-link>
            <span class="mobile-block-text" style="line-height: 16px">【{{ timeDiffFn(detailInfo.serverTime,
              detailInfo.timestamp)
            }}{{ $t('tradeAbout.before') }}】</span>
          </p>
        ...
        <Item :label="$t('blockAbout.blockReward')" :prop="detailInfo.blockReward + ' TURN'"></Item>
        <Item class="extra-data" :label="$t('blockAbout.extraData')" :prop="detailInfo.extraData"></Item>
        <Item v-if="detailInfo.postscript" class="postscript" :label="$t('blockAbout.postscript')">
          <!-- {{ splitFn(detailInfo.postscript) }} -->
          <p class="zero-block" style="white-space: pre-line" v-html="splitFn(detailInfo.postscript)"></p>
        </Item>
      </List>
    </div>
