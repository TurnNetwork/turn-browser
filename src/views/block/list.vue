      ...
      <div class="table-content">
        <el-table :data="tableData" style="width: 100%" key="firstTable" size="mini" v-loading="loading">
          <el-table-column :label="$t('tradeAbout.blockHeight')" width="120px">
            <template slot-scope="scope">
              <div class="flex-special">
                <router-link class="cursor blue ellipsis" :to="getBlockUrl(scope.row.number)">{{ scope.row.number
                }}</router-link>
              </div>
            </template>
          </el-table-column>
          <el-table-column :label="$t('tradeAbout.age')" width="120px">
            <template slot-scope="scope">
              <span>{{ timeDiffFn(scope.row.serverTime, scope.row.timestamp)
          ...
          <el-table-column :label="$t('blockAbout.producer')">
            <template slot-scope="scope">
              <div class="flex-special">
                <router-link class="cursor blue ellipsis ellipsisWidth" :to="getNodeUrl(scope.row.nodeId)">{{
