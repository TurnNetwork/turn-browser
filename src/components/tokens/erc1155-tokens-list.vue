<template>
  <div class="common-trade">
    <div v-if="selectIndex === 1" class="address-trade-last">
      ...
      {{ $t('tokens.typeErc1155Token') }}
      <span style="color: #3F3F3F;" v-if="balanceTotalDisplay > 5000">{{ $t('contract.showingLast') }}</span>
    </div>
    <div v-else class="address-trade-last">
      <!-- 总计代币交易数量 -->
      {{ $t('blockAbout.morethen') }} {{ tradeTotalDisplay }}
      {{ $t('tradeAbout.tokens') }}
      <span style="color: #3F3F3F;" v-if="tradeTotalDisplay > 5000">{{ $t('contract.showingLast') }}</span>
    </div>
    <div class="trade-tab-wrap" v-if="pageType !== 'contract'">
      <ul class="trade-tab">
        <li :class="{ active: selectIndex == 1 }" index="1" @click="typeChange(1, 'blance')">
          {{ $t('tokens.hold1155') }} ({{ balanceTotalDisplay }})
        </li>
    ...
    <div v-show="selectIndex === 1" class="table">
      <el-table :data="balanceTableData" style="width: 100%" key="firstTable" size="mini" v-loading="loading.blance">
        ...
        <el-table-column :label="$t('tokens.tokenID')">
          <template slot-scope="scope">
            <router-link :to="get1155IdUrl(scope.row.contract, scope.row.tokenId)" class="cursor normal">
              {{ scope.row.tokenId }}
            </router-link>
          </template>
        </el-table-column>
      ...
      <el-table :data="tradeTableData" style="width: 100%" key="secondTable" size="mini" v-loading="loading.trade">
        ...
        <template v-if="pageType === 'contract'">
          ...
          <el-table-column prop="blockHeight" :label="$t('tradeAbout.block')" width="120">
            <template slot-scope="scope">
              <router-link class="cursor blue" :to="getBlockUrl(scope.row.blockNumber)">
        ...
        <el-table-column v-else :label="$t('tradeAbout.age')" width="120">
          <template slot-scope="scope">
            <span>
              {{ timeDiffFn(scope.row.blockTimestamp, scope.row.systemTimestamp) }}{{ $t('tradeAbout.before') }}
            </span>
          </template>
        </el-table-column>
        ...
        <template v-if="pageType === 'contract'">
          <el-table-column :label="$t('tokens.tokenID')">
            <template slot-scope="scope">
              <router-link class="cursor normal ellipsis ellipsisWidth" :to="get1155IdUrl(scope.row)">
                {{ scope.row.tokenId }}
              </router-link>
            </template>
          </el-table-column>
      ...
      apiService.tokens
        ...
        .then(res => {
          ...
          if (code == 0) {
            ...
            this.tradePageTotal = this.tradeTotalDisplay = this.tradeCount.erc1155TxQty // || displayTotalCount;
          } else {
            this.$message.error(errMsg)
          }
        })
