<template>
  <div>
    <div class="address-trade-last">
      ...
      <span style="color: #3f3f3f">{{ tradeCount.haveReward | formatMoney }} TURN</span>
    </div>
    ...
    <div class="detail-box" v-for="item in rewardTableData" :key="item.id">
      ...
      <div class="table-box">
        <div class="total">
          ...
          <span class="value">{{ item.allRewards | formatMoney }} TURN</span>
        </div>
        <div class="reward-box">

          <div class="reward-box-line" v-for="(ele, $index) in item.rewardsDetails" :key="$index">
            <router-link :to="getNodeUrl(ele.verify)" class="td-title cursor normal ellipsis">
              {{ ele.nodeName }}
            </router-link>
            <div class="td-content">
              {{ ele.reward | formatMoney }} &nbsp;&nbsp;TURN
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="rewardTableData.length !== 0" class="pagination-box">
      <el-pagination background :total="totalNum" :current-page="curPage" :page-size="pageSize"
        :page-sizes="[10, 20, 50, 100]" @size-change="handleSizeFn" @current-change="handleChange"
        layout="sizes,total,prev,pager,next"></el-pagination>
    </div>
    <!-- {{ tradeCount }} -->
    <!-- {{ address }} -->
  </div>
