    ...
    <div class="address-delegation">
      <ul>
        ...
        <li>
          ...
          <p>{{ $t("contract.totalDelegated") }}(TURN)</p>
        </li>
        <li>
        ...
        <li>
          ...
          <p>{{ $t("deleget.lockedDelegate") }}(TURN)</p>
        </li>
        <li>
          <span>{{ detailInfo.delegateHes | formatMoney }}</span>
          <p>{{ $t("deleget.unlockedDelegate") }}(TURN)</p>
        </li>
      </ul>
    </div>

    <div class="table gray-table">
      <el-table :data="tableData" style="width: 100%;min-width: 910px;" key="firstTable" size="mini">
        <el-table-column :label="$t('nodeInfo.validator')">
          <template slot-scope="scope">
            <div class="flex-special">
              <router-link class="cursor blue ellipsis" :to="getNodeUrl(scope.row.nodeId)">
                {{ scope.row.nodeName }}
              </router-link>
            </div>
          </template>
