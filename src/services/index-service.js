...
class IndexService extends Ws {
    ...
    getMicroNodeStatistic() {
        sub.addSub(() => {
            // console.log('this.stompClient', this.stompClient);
            this.stompClient.subscribe(API.WS_CONFIG.micronodestatistic, (msg) => {
                const res = JSON.parse(msg.body)
                const { data, code } = res
                console.log(`micronodestatistic`, res)
                if (code === 0) {
                    store.dispatch('updateMicroNodeData', data)
                } else {
                    throw new Error(`todo`)
                }
            })
